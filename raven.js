...
(function () {
  ...
  function init() {
    ...
    const specGhost = $(".spec-ghost");
    if (specGhost) {
      gsap.fromTo(specGhost, { yPercent: 14 }, {
        yPercent: -14, ease: "none",
        scrollTrigger: { trigger: ".spec", start: "top bottom", end: "bottom top", scrub: true },
      });
    }

    // cinematic wipe reveal — shared by every full-bleed dark section
    [".perf-media", ".craft-media", ".crew-media"].forEach((sel) => {
      const el = $(sel);
      if (!el) return;
      gsap.fromTo(el,
    ...
    if (!isTouch) {
      $$(".performance, .craft, .crew").forEach((section) => {
        section.addEventListener("mousemove", (e) => {
          ...
          section.style.setProperty("--gy", ((e.clientY - r.top) / r.height * 100).toFixed(1) + "%");
        });
      });
    }
