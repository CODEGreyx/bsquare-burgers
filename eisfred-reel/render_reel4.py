...
w_fresh  = word("Fresh.", F_SERIF, 112, 1, INK, shadow=False, period_gold=False)
w_daily  = word("Daily.", F_SERIF, 112, 1, INK, shadow=False, period_gold=False)
...
# ---------- real footage kept from the uploaded commercial (fakes discarded) ----------
SEG_DIR = "/tmp/claude-0/-home-user-bsquare-burgers/292677ef-0b3f-5726-8ed6-1e15356b92b8/scratchpad"
SEG_N = {"seg1": 42, "seg4": 44}
_seg_cache = {}
def seg_frame(name, i):
    idx = min(i+1, SEG_N[name])
    key = (name, idx)
    if key not in _seg_cache:
        im = Image.open(f"{SEG_DIR}/{name}/f_{idx:03d}.png").convert("RGB")
        _seg_cache[key] = im.filter(ImageFilter.UnsharpMask(radius=1.5, percent=45, threshold=2))
    return _seg_cache[key]

# ---------- six scenes on the 120 BPM grid ----------
# S1 0-1.5 storefront clip | S2 1.5-3.5 interior "Handcrafted" | S3 3.5-5.5 street "in Berlin."
# S4 5.5-7.5 kiosk "Fresh."+"Daily." | S5 7.5-9.0 scoop clip | S6 9.0-12.0 brand card
SCENE_T = {1:(0.0,1.5), 2:(1.5,3.5), 3:(3.5,5.5), 4:(5.5,7.5), 5:(7.5,9.0), 6:(9.0,12.0)}

def shot(idx, t01, f=0):
    a, b = SCENE_T[idx]
    tsec = a + t01*(b-a)

    if idx == 1:  # real storefront footage (kept from the uploaded spot)
        fr = seg_frame("seg1", f)
        s = 1.12 + 0.03*t01          # crop past the clip's baked-in border vignette
        cw, ch = W/s, H/s
        fr = fr.resize((W, H), Image.LANCZOS, box=((W-cw)/2, (H-ch)/2, (W+cw)/2, (H+ch)/2))
        fr = grade(fr, grain=1.6).convert("RGBA")
        paste_center(fr, chrome, W/2, 130, alpha=clamp(tsec/0.4, 0, 1))
        return fr

    if idx == 2:  # interior — "Handcrafted" (ink on the cream wall)
        fr = fullshot("interior", t01, 1.06, 1.17, (640, 510), (665, 520))
        fr = grade(fr).convert("RGBA")
        paste_center(fr, chrome_d, W/2, 130)
        reveal(fr, w_hand, W/2, H*0.30, 1.85, tsec)
        return fr

    if idx == 3:  # street — "in Berlin." (white over the pavement)
        fr = fullshot("street", t01, 1.02, 1.14, (543, 730), (543, 650))
        fr = grade(fr).convert("RGBA")
        fr = scrim(fr, W/2, H*0.78, rad=430, dark=0.34)
        paste_center(fr, chrome, W/2, 130)
        reveal(fr, w_berlin, W/2, H*0.78, 3.85, tsec)
        return fr

    if idx == 4:  # kiosk — "Fresh." then "Daily." on the yellow awning
        fr = fullshot("kiosk", t01, 1.04, 1.16, (543, 700), (543, 660))
        fr = grade(fr).convert("RGBA")
        paste_center(fr, chrome, W/2, H-110)
        reveal(fr, w_fresh, W/2, H*0.095, 5.85, tsec)
        reveal(fr, w_daily, W/2, H*0.172, 6.40, tsec)
        return fr

    if idx == 5:  # real scooping footage (kept), the product moment — no words
        fr = seg_frame("seg4", f)
        return grade(fr, grain=1.6).convert("RGBA")
...
BOUNDS = [(1,0,45),(2,45,105),(3,105,165),(4,165,225),(5,225,270),(6,270,360)]

def shot_at(f):
    for idx, a, b in BOUNDS:
        if a <= f < b:
            return idx, (f-a)/(b-a-1), f-a
    return 6, 1.0, 89

def render_frame(f):
    idx, t01, fl = shot_at(f)
    fr = shot(idx, t01, fl)

    # flash-frame into the product reveal (frames 223..229)
    if 223 <= f < 229:
        base = fr.convert("RGB")
        w = 0.45*(f-222)/2.0 if f < 225 else max(0.0, 0.80-0.28*(f-225))
    ...
    # gentle dissolve into brand card (frames 266..276)
    if 266 <= f < 276:
        k = (f-266)/10.0
        if f < 270:
            fr = zoom_blur(fr.convert("RGB"), 0.012*k).convert("RGBA")
        else:
            prev = zoom_blur(shot(5, 1.0, 44).convert("RGB"), 0.018)
